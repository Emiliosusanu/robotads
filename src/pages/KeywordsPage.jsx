  ...
  for (let i = 0; i < count; i++) {
    ...
    keywords.push({
      ...
      text:
        ...
        (matchTypes[i % matchTypes.length] === "exact"
          ? ` [${i + 1}]`
          : ` ${i + 1}`),
...
const KeywordsPage = () => {
  ...
  const fetchAdGroupsForAccount = useCallback(async () => {
    ...
    try {
      ...
      toast({
        title: "Error fetching ad groups for account",
        description: error.message,
        variant: "destructive",
      });
      setAdGroups([]);
  ...
  const fetchKeywords = useCallback(async () => {
    ...
    setShowSimulatedData(false);
    try {
      const currentAmazonAccount = linkedAccounts.find(
        (acc) => acc.id === selectedAccountId
      );
  ...
  useEffect(() => {
    if (
      selectedAccountId &&
      user &&
      (selectedAdGroupId || adGroups.length === 0)
    ) {
      fetchKeywords();
  ...
  const handleSyncData = async (accountId) => {
    ...
    toast({
      ...
      description:
        "Fetching latest Keyword data from Amazon. This may take a few minutes.",
    });
    try {
      ...
      toast({
        title: "Sync Error",
        description: `Failed to sync keyword data: ${errorMessage}`,
        variant: "destructive",
      });
      setShowSimulatedData(true);
      if (
  ...
  const displayedKeywords = showSimulatedData
    ? generateSimulatedKeywords(20, selectedAdGroupId || "sim-ag-1")
    : keywords;

  const filteredKeywords = displayedKeywords.filter(
    (kw) =>
      (kw.text && kw.text.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (kw.amazon_keyword_id &&
        kw.amazon_keyword_id.toLowerCase().includes(searchTerm.toLowerCase()))
  );
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <CardDescription className="text-slate-400">
                View and manage your Amazon Advertising keywords. Last sync:{" "}
            ...
            <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
              {loadingAccounts ? (
                ...
                <Select
                  ...
                  disabled={loadingAdGroups || loadingData}
                >
          ...
          currentAccountStatus !== "error_no_region" ? (
            ...
            <div className="text-center py-10">
              ...
              <p className="text-slate-400">
                No keywords for the selected criteria, or data hasn't been
                synced.
              </p>
              <Button
                onClick={() => handleSyncData(selectedAccountId)}
              ...
              <Table>
                ...
                <TableBody>
                  {filteredKeywords.map((kw) => (
                    ...
                    >
                      <TableCell
                        className="font-medium text-purple-300 min-w-[150px] break-all"
                        title={kw.text}
                      >
                        {kw.text}{" "}
                        <span className="text-xs text-slate-500">
                          ({kw.amazon_ad_groups?.name || "N/A"})
                        </span>
                      </TableCell>
                      ...
                      <TableCell>
                        {formatCurrency(kw.raw_data?.spend)}
                      </TableCell>
                      <TableCell>
                        {formatNumber(kw.raw_data?.impressions)}
                      </TableCell>
                      <TableCell>{formatNumber(kw.raw_data?.clicks)}</TableCell>
                      <TableCell>
                        {formatPercentage(
                          kw.raw_data?.impressions > 0
                            ? (kw.raw_data?.clicks || 0) /
                                kw.raw_data.impressions
                            : 0
                        )}
                      </TableCell>
                      <TableCell>
                        {formatCurrency(
                          kw.raw_data?.clicks > 0
                            ? (kw.raw_data?.spend || 0) / kw.raw_data.clicks
                            : 0
                        )}
                      </TableCell>
                      ...
                      <TableCell>
                        {formatPercentage(
                          kw.raw_data?.sales > 0
                            ? (kw.raw_data?.spend || 0) / kw.raw_data.sales
                            : 0
                        )}
                      </TableCell>
                    </TableRow>
