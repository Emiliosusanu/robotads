...
const generateSimulatedCampaigns = (count = 10) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    campaigns.push({
      ...
      raw_data: {
        // Simulating some raw_data fields for display
        spend: spend,
        orders: orders,
...
const CampaignsPage = () => {
  ...
  const fetchCampaigns = useCallback(async () => {
    ...
    try {
      ...
      if (error) throw error;
      setCampaigns(data || []);
      if ((data || []).length === 0 && currentAccountStatus === "active") {
        // If active account has no campaigns, show simulated
  ...
  const handleSyncData = async (accountId) => {
    ...
    toast({
      ...
      description:
        "Fetching latest campaign data from Amazon. This may take a few minutes.",
    });
    try {
      ...
      toast({
        title: "Sync Error",
        description: `Failed to sync campaign data: ${errorMessage}`,
        variant: "destructive",
      });
  ...
  const displayedCampaigns = showSimulatedData
    ? generateSimulatedCampaigns()
    : campaigns;

  const filteredCampaigns = displayedCampaigns.filter(
    (campaign) =>
      (campaign.name &&
        campaign.name.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (campaign.amazon_campaign_id_text &&
        campaign.amazon_campaign_id_text
          .toLowerCase()
        ...
        {linkedAccounts.length > 0 &&
          ...
          currentAccountStatus !== "error_no_region" && (
            <Button
              ...
              disabled={
                syncingAccount === selectedAccountId || loadingCampaigns
              }
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <CardDescription className="text-slate-400">
                View and manage your Amazon Advertising campaigns. Last sync:{" "}
          ...
          currentAccountStatus !== "error_no_region" ? (
            ...
            <div className="text-center py-10">
              ...
              <p className="text-slate-300 text-lg">
                Please select an Amazon account to view campaigns.
              </p>
            ...
            <div className="text-center py-10">
              ...
              <p className="text-slate-400 mb-4">
                {currentAccountStatus === "reauth_required" &&
                  "The access token for this Amazon account is invalid. Please re-link it."}
                {currentAccountStatus === "error_no_profile" &&
                ...
                <TableBody>
                  {filteredCampaigns.map((campaign) => (
                    ...
                    >
                      ...
                      <TableCell>
                        {formatCurrency(campaign.raw_data?.spend)}
                      </TableCell>
                      <TableCell>
                        {formatNumber(campaign.raw_data?.impressions)}
                      </TableCell>
                      <TableCell>
                        {formatNumber(campaign.raw_data?.clicks)}
                      </TableCell>
                      <TableCell>
                        {formatPercentage(
                          campaign.raw_data?.impressions > 0
                            ? (campaign.raw_data?.clicks || 0) /
                                campaign.raw_data.impressions
                            : 0
                        )}
                      </TableCell>
                      <TableCell>
                        {formatCurrency(
                          campaign.raw_data?.clicks > 0
                            ? (campaign.raw_data?.spend || 0) /
                                campaign.raw_data.clicks
                            : 0
                        )}
                      </TableCell>
                      <TableCell>
                        {formatNumber(campaign.raw_data?.orders)}
                      </TableCell>
                      <TableCell>
                        {formatPercentage(
                          campaign.raw_data?.sales > 0
                            ? (campaign.raw_data?.spend || 0) /
                                campaign.raw_data.sales
                            : 0
                        )}
                      </TableCell>
                    </TableRow>
