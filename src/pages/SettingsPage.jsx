...
const SettingsPage = () => {
  ...
  const [currentRule, setCurrentRule] = useState(
    JSON.parse(JSON.stringify(initialRuleState))
  );
  ...
  const handleSubmit = async (ruleDataToSave) => {
    if (!user) {
      toast({
        ...
        description: "Please login to save rules.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (isEditing && editingRuleId) {
        response = await supabase
          .from("optimization_rules")
          .update(finalRuleData)
          .eq("id", editingRuleId)
          .select();
      } else {
        response = await supabase
          .from("optimization_rules")
          .insert(finalRuleData)
          .select();
      }
      ...
      toast({
        title: `Rule ${isEditing ? "Updated" : "Created"}`,
        description: `Rule "${data[0].name}" saved successfully.`,
        variant: "default",
      ...
      if (accError || !accounts?.length) {
        console.warn("No linked Amazon account found.");
        return;
      }
  ...
  const handleDelete = async (ruleId) => {
    ...
    try {
      ...
      if (editingRuleId === ruleId) {
        // If deleting the rule currently in edit form
        setShowForm(false);
        setIsEditing(false);
        setEditingRuleId(null);
        setCurrentRule(JSON.parse(JSON.stringify(initialRuleState)));
  ...
  const toggleRuleEnabled = async (rule) => {
    ...
    try {
      ...
      if (error) throw error;
      toast({
        title: `Rule ${data[0].enabled ? "Enabled" : "Disabled"}`,
        variant: "default",
      });
      fetchRules();
    } catch (error) {
      toast({
        title: "Error updating rule status",
        description: error.message,
        variant: "destructive",
      });
    } finally {
