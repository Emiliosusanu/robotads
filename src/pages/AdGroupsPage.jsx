...
const generateSimulatedAdGroups = (
  count = 15,
  ...
  const names = [
    ...
    "Category Targeting - Books",
    "Auto Targeting - Loose Match",
    "Manual Bid Group",
    "Top Performers",
    "New Keywords Test",
  ];
...
const AdGroupsPage = () => {
    ...
    if (currentUser) {
      try {
        ...
        if (data && data.length > 0) {
          const activeAccount =
            data.find((acc) => acc.status === "active") || data[0];
  ...
  const fetchCampaignsForAccount = useCallback(async () => {
    ...
    try {
      ...
      toast({
        title: "Error fetching campaigns for account",
        description: error.message,
        variant: "destructive",
      });
      setCampaigns([]);
  ...
  useEffect(() => {
    if (selectedAccountId && user) {
      ...
      const selectedAcc = linkedAccounts.find(
  ...
  const fetchAdGroups = useCallback(async () => {
    ...
    try {
      ...
      toast({
        title: "Error fetching ad groups",
        description: error.message,
        variant: "destructive",
      });
      setAdGroups([]);
  ...
  const handleSyncData = async (accountId) => {
    ...
    toast({
      ...
      description:
        "Fetching latest Ad Group data from Amazon. This may take a few minutes.",
    });
    try {
      ...
      const { data: funcData, error: funcError } =
        await supabase.functions.invoke("fetch-amazon-data", {
      ...
      toast({
        title: funcData?.success ? "Sync Successful" : "Sync Info",
        description: message,
        variant: variant,
        className: funcData?.success ? "bg-green-600 text-white" : "",
      });
      ...
      const updatedAccounts = linkedAccounts.map((acc) =>
        acc.id === accountId
          ? { ...acc, last_sync: new Date().toISOString(), status: newStatus }
          : acc
      );
      ...
      const errorMessage =
        error.message || error.context?.message || "Unknown error during sync.";
      toast({
        title: "Sync Error",
        description: `Failed to sync ad group data: ${errorMessage}`,
        variant: "destructive",
      });
      setShowSimulatedData(true);
      if (
        ...
        error.message.includes("Amazon Profile ID is missing")
      ) {
        const updatedAccounts = linkedAccounts.map((acc) =>
          acc.id === accountId ? { ...acc, status: "reauth_required" } : acc
        );
        setLinkedAccounts(updatedAccounts);
        if (accountId === selectedAccountId)
          setCurrentAccountStatus("reauth_required");
      }
    } finally {
      setSyncingAccount(null);
    }
  ...
  const displayedAdGroups = showSimulatedData
    ? generateSimulatedAdGroups(15, selectedCampaignId || "sim-campaign-1")
    : adGroups;

  const filteredAdGroups = displayedAdGroups.filter(
    (ag) =>
      (ag.name && ag.name.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (ag.amazon_ad_group_id &&
        ag.amazon_ad_group_id.toLowerCase().includes(searchTerm.toLowerCase()))
  );
  ...
  const getSortIcon = (key) => {
    ...
    return sortConfig.direction === "ascending" ? (
      <ArrowUpDown
        size={14}
        className="ml-2 text-purple-400 transform rotate-180"
      />
    ) : (
    ...
    >
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        {linkedAccounts.length > 0 &&
          selectedAccountId &&
          currentAccountStatus !== "reauth_required" &&
          currentAccountStatus !== "error_no_profile" &&
          currentAccountStatus !== "error_no_region" && (
            <Button
        ...
        {(currentAccountStatus === "reauth_required" ||
          ...
          selectedAccountId && (
            <Button
              onClick={() =>
                navigate("/link-amazon", {
                  state: {
                    accountIdToRelink: selectedAccountId,
                    client_id: linkedAccounts.find(
                      (acc) => acc.id === selectedAccountId
                    )?.client_id,
                  },
                })
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <CardDescription className="text-slate-400">
                View and manage your Amazon Advertising ad groups. Last sync:{" "}
                {linkedAccounts.find((acc) => acc.id === selectedAccountId)
                  ?.last_sync
                  ? new Date(
                      linkedAccounts.find(
                        (acc) => acc.id === selectedAccountId
            ...
            <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
              {loadingAccounts ? (
                ...
                <Select
                  ...
                  disabled={
                    loadingCampaigns || loadingData || syncingAccount !== null
                  }
                >
                ...
                <p className="text-yellow-400 text-sm self-center">
                  No Amazon accounts linked.{" "}
                  <Button
                    variant="link"
                    className="p-0 h-auto text-purple-400"
                    onClick={() => navigate("/link-amazon")}
                  >
                    Link an account
                  </Button>
                </p>
              ...
              {selectedAccountId &&
                (loadingCampaigns ? (
                  ...
                  <Select
                    ...
                    disabled={loadingData || syncingAccount !== null}
                  >
                    <SelectTrigger className="w-full sm:w-[200px] bg-slate-700 border-slate-600 text-slate-100">
        ...
        <CardContent>
          ...
          currentAccountStatus !== "error_no_region" ? (
            ...
            <div className="text-center py-10">
              <AlertTriangle
                size={48}
                className="mx-auto text-yellow-400 mb-4 opacity-70"
              />
              <p className="text-slate-300 text-lg">
                Please select an Amazon account.
              </p>
              <p className="text-slate-400 text-sm mt-1">
                If no accounts are linked, simulated data will be shown.
              </p>
            </div>
          ) : (currentAccountStatus === "reauth_required" ||
              currentAccountStatus === "error_no_profile" ||
            ...
            <div className="text-center py-10">
              <AlertTriangle
                size={48}
                className="mx-auto text-red-500 mb-4 opacity-70"
              />
              <h3 className="text-xl font-semibold text-slate-200 mb-2">
                ...
                {currentAccountStatus === "error_no_profile" &&
                  "Profile ID Missing"}
                {currentAccountStatus === "error_no_region" &&
                  "Region Invalid/Missing"}
              </h3>
              <p className="text-slate-400 mb-4">
                {currentAccountStatus === "reauth_required" &&
                  "The access token for this Amazon account is invalid or key information is missing. Please re-link it."}
                {currentAccountStatus === "error_no_profile" &&
                  "The Amazon Profile ID is missing for this account. Please re-link to configure it."}
                {currentAccountStatus === "error_no_region" &&
                  "The Amazon Region is missing or invalid for this account. Please re-link to configure it."}
              </p>
              <Button
                onClick={() =>
                  navigate("/link-amazon", {
            ...
            <div className="text-center py-10">
              ...
              <p className="text-slate-400">
                No ad groups for the selected criteria, or data hasn't been
                synced.
              </p>
              <Button
                onClick={() => handleSyncData(selectedAccountId)}
              ...
              <Table>
                  ...
                  <TableRow className="border-slate-700">
                    {[
                      ...
                      <TableHead
                        ...
                        className="cursor-pointer hover:bg-slate-700/50 transition-colors text-slate-300"
                      >
                ...
                <TableBody>
                  {filteredAdGroups.map((ag) => (
                    ...
                    >
                      <TableCell
                        className="font-medium text-purple-300 min-w-[200px] break-all"
                        title={ag.name}
                      >
                        {ag.name}{" "}
                        <span className="text-xs text-slate-500">
                          ({ag.amazon_campaigns?.name || "N/A"})
                        </span>
                      </TableCell>
                      <TableCell>
                        <span
                      ...
                      <TableCell>
                        {formatCurrency(ag.raw_data?.spend)}
                      </TableCell>
                      <TableCell>
                        {formatNumber(ag.raw_data?.impressions)}
                      </TableCell>
                      <TableCell>{formatNumber(ag.raw_data?.clicks)}</TableCell>
                      <TableCell>
                        {formatPercentage(
                          ag.raw_data?.impressions > 0
                            ? (ag.raw_data?.clicks || 0) /
                                ag.raw_data.impressions
                            : 0
                        )}
                      </TableCell>
                      <TableCell>
                        {formatCurrency(
                          ag.raw_data?.clicks > 0
                            ? (ag.raw_data?.spend || 0) / ag.raw_data.clicks
                            : 0
                        )}
                      </TableCell>
                      ...
                      <TableCell>
                        {formatPercentage(
                          ag.raw_data?.sales > 0
                            ? (ag.raw_data?.spend || 0) / ag.raw_data.sales
                            : 0
                        )}
                      </TableCell>
                    </TableRow>
